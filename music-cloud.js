(function () {
  ...
  function comparableMusicName(value) {
    return String(value || "").toLocaleLowerCase().replace(/\band\b/g, "").replace(/[^a-z0-9]+/g, "");
  }
  ...
  class MusicCloud {
      ...
      try {
        const metadataResponse = await fetch(`${PROJECT_URL}/rest/v1/music_tracks`, {
          ...
          body: JSON.stringify({
            ...
            source_metadata: identity.sourceMetadata,
          }),
        });
    ...
    async updateTrackMetadata(track, title, artist) {
      ...
      const sourceMetadata = track?.source_metadata && typeof track.source_metadata === "object" && !Array.isArray(track.source_metadata)
        ...
        : { artist: cleanArtist };
      const response = await fetch(`${PROJECT_URL}/rest/v1/music_tracks?id=eq.${encodeURIComponent(track.id)}&user_id=eq.${encodeURIComponent(this.user.id)}`, {
        method: "PATCH",
        headers: this.headers({ "Content-Type": "application/json", Prefer: "return=representation" }, true),
        body: JSON.stringify({ title: cleanTitle, source_metadata: sourceMetadata }),
      });
      const updated = (await readResponse(response))?.[0];
      if (!updated) throw new Error("The song could not be updated.");
      return updated;
    }
    ...
    async deleteTracks(trackList) {
      ...
      for (let start = 0; start < uniqueTracks.length; start += 1000) {
        ...
        await readResponse(await fetch(`${PROJECT_URL}/storage/v1/object/${BUCKET}`, {
          method: "DELETE",
          headers: this.headers({ "Content-Type": "application/json" }, true),
          body: JSON.stringify({ prefixes: batch.map((track) => track.storage_path) }),
        }));
      }
      for (let start = 0; start < uniqueTracks.length; start += 200) {
        const ids = uniqueTracks.slice(start, start + 200).map((track) => track.id).join(",");
        await readResponse(await fetch(`${PROJECT_URL}/rest/v1/music_tracks?id=${encodeURIComponent(`in.(${ids})`)}`, {
          method: "DELETE",
