...
The reference gateway supports Wikimedia, YouTube Data, NASA Images, TMDB, Open Library, REST Countries, and MusicBrainz. Public providers still run through the gateway so CORS, timeouts, error isolation, output limits, and response normalization are consistent. YouTube and TMDB credentials belong only in Supabase Edge Function secrets (`YOUTUBE_API_KEY` plus `TMDB_API_KEY` or `TMDB_API_TOKEN`). REST Countries v5 requires its free account key in `REST_COUNTRIES_API_KEY`; do not use the retired v3.1 endpoint or treat its HTTP 200 deprecation body as search data. Never place these credentials in `index.html`, frontend JavaScript, GitHub Actions output, or repository files.
...
| --- | --- | --- | --- | --- |
| 2026-08-04 | REST Countries appeared healthy but always returned zero results. | The retired v3.1 endpoint returned HTTP 200 with a deprecation error object, so generic HTTP success handling mistook it for an empty valid response. | Use only the current v5 endpoint with `REST_COUNTRIES_API_KEY`; when the key is absent, return an explicit setup state and provider link instead of calling a retired API. | Search REST Countries for Canada: with a configured key, confirm Canada/Ottawa render; without one, confirm a clickable setup state and no false healthy status. |
