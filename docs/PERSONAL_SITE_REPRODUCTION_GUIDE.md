...
- Separate cumulative language tools from lesson-specific practice. Cards, the complete pronunciation chart, the combined Words/Characters grid, Writing, and Quiz aggregate across lessons. A selected lesson owns only its overview plus a single Practice surface that switches among Sentences, Conversation, and Reading.
...
- Keep the language study menu structurally identical on notebook and quiz routes. Voice Settings is one top-corner popover control, not a study-menu item. Writing remains present on every route.
...
| --- | --- | --- | --- | --- |
| 2026-08-08 | Language tools could not scale cleanly to Lesson 2: Words, Cards, and Characters were bound to the selected lesson, while Sentences, Conversation, and Reading occupied separate routes. | Cumulative knowledge and lesson-specific practice were rendered from the same route-level lesson variables. | Cards and Words aggregate all lesson data; Characters becomes a Words filter; mastery colors derive from the two synced known lists; Sentences, Conversation, and Reading share one lesson-specific layered-reveal Practice route. Sounds is one global complete Pinyin component chart. | Verify 166 cumulative Lesson 1 words plus combined character entries, 23/24/16/5 Sounds groups, yellow/orange/purple filters, a visible known-word panel, 30 conversation turns, 5 readings, and both reveal directions. Add a sample Lesson 2 object and confirm cumulative counts grow while Lesson 1 Practice remains unchanged. |
...
| 2026-08-08 | Writing disappeared from the Mandarin section menu after opening Quiz. | The notebook and Quiz duplicated their navigation markup, and the Quiz copy omitted Writing while also retaining outdated Home/Plans labels. | Mandarin notebook and Quiz expose the same study destinations and labels; remove Mandarin Home, use `Lesson 1` and `Conversation`, and include Writing in both menus. | Open Quiz directly and from every notebook page; confirm Lesson 1, Cards, Sounds, Words, Writing, Sentences, Conversation, Reading, Characters, and Quiz are present exactly once. |
